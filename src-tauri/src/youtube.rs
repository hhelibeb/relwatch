...
///
/// 行为与 `github::save_releases` 对齐：按 published_at 降序排列，`max_count=1`
/// 时遇到已入库记录立即返回空；历史模式跳过已存在记录继续。
pub fn save_entries(
    conn: &Connection,
    source_id: i64,
    ...
    let mut entries: Vec<FeedEntry> = items
        .iter()
    ...
    for entry in &entries {
        ...
        if let Ok(id) = releases::insert_release(
            ...
            if id > 0 {
                ...
                saved.push((id, entry.description.clone()));
