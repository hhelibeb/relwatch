...
> 随版本线性膨胀（实测：v1.15.0 均长 33 字 → v1.17.2 均长 172 字、最长 326 字，
> 单版总字数 2237）。
...
| 项 | 上限 | 说明 |
|---|---|---|
| 单条字符数 | **≤ 100 字**（含标点，不含 hash） | 约一行手机屏宽；超过就说明在讲实现 |
| 单版总字数 | **≤ 800 字** | 条数由变更量决定，不砍条数——超限说明单条写太长 |
| 单版最多挑 3 条 | 各配一句「为什么重要」 | 只给最值得用户注意的改动，其余一律一句结论 |
...
上例由 v1.17.3 实战产出：该版 10 条 / 1911 字压缩为 **8 条 / 460 字**后通过检查
（条数只减 2，压缩主要来自单条瘦身——修掉实现细节与根因叙述，用户关心的结论一条没少）。

> ⚠️ **自检命令**：写完 CHANGELOG 后跑一次，超限必须先压缩再进入 Step 4.3：
>
> ```bash
> node scripts/check-changelog-length.mjs <新版本>
> # 退出码 0 通过 / 1 超限；检查单条 ≤100 字、单版总字数 ≤800 字
> ```

**例外**：涉及用户必须采取行动的安全更新（如凭据泄露、需改配置的漏洞）可放宽到
200 字，但仍要写成「发生了什么 + 用户要做什么」，不展开内部机制。
